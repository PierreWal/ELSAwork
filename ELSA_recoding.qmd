...
rslt.r <- lapply(c(uvars,dvars), function(v) wfreq2(v, elsaw.s) )


# rslt.u <- lapply(uvars, ufreq)
...
for (i in 1:length(rslt.u)) {
   cat('### ', labs[[i]], '\n')
    ...
    cat(paste0("#### **Unweighted frequency table of unrecoded *",uvars[i],"* **", '\n'))
    print(rslt.u[[i]])
    cat('\n')
    
    # cat(paste0("#### **Weighted frequency table of unrecoded *",uvars[i],"* **", '\n'))
    # print(knitr::kable(rslt.r[[i]]),'html')
    #  cat('\n')
     
    cat(paste0("#### **Weighted frequency table of recoded *", uvars[i],"* **", '\n'))
    print(rslt.r[[i]])
    cat('\n')
...
print(kable(
round((table(Age=elsaw$AgeCat2.fr,Sex=elsaw$indsex.fr,"Marital status"=elsaw$dimarr.fr, "Voted"=elsaw$VotGE19.fr))) 
),'html')
```
