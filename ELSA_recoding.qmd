...
subtitle: Exploratory analysis  - draft version 6
...
1. Search and add missing variables (Interest in politics; contacted MP)
...
ovars<-c("cfmap", "cfmetm", "digran", "dimarr", "edqual", "erfvol","erlvolba", "erlvolpe", "ethnic","exlo80","exlo90", "exrela", "headlba","headlbe","hehelf", "hhdtypb","hobb","iafcon", "iafind","indsex", "psagf", "pscedd", "pscede","sclfcoa", "scorgch", "scorgpo","scorgrl","spcar",  "sptraa", "tenureb", "VotGE01","VotGE05","VotGE15","VotGE17","VotGE19","w10nssec5","wpdes")    
...
bivars19<-c("AgeCat2","AgeCat3", "cfmap", "cfmetm","digran", "dimarr", "edqual", "erfvol", "erlvolba","erlvolpe","ethnic", "exrela", "headlba","headlbe","hehelf", "hhdtypb","hobb","iafcon", "iafind","indsex", "pscedd", "pscede", "sclfcoa",  "scorgch","scorgpo","scorgrl","spcar",  "sptraa", "tenureb","VotGE01","VotGE05","VotGE15","VotGE17","VotRec0","VotRec01","w10nssec5","wpdes")     
...
.default = NA
    ...
    cfmetm2.f=as.ordered(case_when(
...
.default = NA
  ...
  erlvolpe.f=as.ordered(case_when(
    erlvolpe==1  ~ "Cared for sick/frail",
    erlvolpe==0 ~ "Not a carer"
  )),
erlvolba.f=as.ordered(case_when(
  ...
  hehelf2.f=as.ordered(case_when(
    hehelf==1  | hehelf==2 ~ "Excel/V good",
    hehelf==3 ~ "Good",
  ...
  hhdtypb.f=as.ordered(case_when(
    ...
    sclfcoa.f=as.ordered(case_when(
    ...
    scorgch.f=as.ordered(case_when(
...
```{r miscrec}
for(v  in ovars[-c(21,11,10)]) { ### Continuous vars exlo870 & exlo90 removed
nv<-paste0(v,".fr")
ov<-paste0(v,".f")
...
|Name|Description|Variable label|Valid responses|
|----|-----------|------------------------------------|-----------:
Age  2 categories  | AgeCat2    | `{r} attr(elsaw$AgeCat2, "label")` |`{r} prettyNum(as.numeric(table(is.na(elsaw$AgeCat2)))[1],big.mark = ",")` 
Age 3 categories  | AgeCat3    | `{r} attr(elsaw$AgeCat3, "label")` |`{r} prettyNum(as.numeric(table(is.na(elsaw$AgeCat3)))[1],big.mark = ",")` 
...
General Health| HEHELF|`{r} attr(elsaw$hehelf, "label")` |`{r} prettyNum(as.numeric(table((elsaw$hehelf.fr)))[1],big.mark = ",")` 
...
Financial security|IAFCON,IAFINS,EXRELA|`{r}  attr(elsaw$ervol, "label")` |`{r} prettyNum(as.numeric(table(is.na(elsaw$exrela.fr)))[1],big.mark = ",")` 
Control|IAFIND|`{r}  attr(elsaw$iafind, "label")` |`{r} prettyNum(as.numeric(table(is.na(elsaw$iafind.fr)))[1],big.mark = ",")` 
...
rslt.u<-  lapply(c(uvars[-c(21,11,10)],dvars), function(v) vfreq(v, elsaw))

rslt.r <- lapply(c(rvars[-c(21,11,10)],dvars), function(v) wfreq2(v, elsaw.s) )



# Print the tables for each dependent variable using kable for better formatting
for (i in 1:length(rslt.u)) {
   cat('### ', labs[[c(uvars[-c(21,11,10)],dvars)[i]]], '\n')
  
    cat(paste0("#### **Unweighted frequency table of unrecoded *",c(uvars[-c(21,11,10)],dvars)[i],"* **", '\n'))
    print(rslt.u[[i]])
    cat('\n')
    

    cat(paste0("#### **Weighted frequency table of recoded *", c(rvars[-c(21,11,10)],dvars)[i],"* **", '\n'))
    print(rslt.r[[i]])
...
summary(elsaw$psagf[elsaw$psagf>0])  
cat('\n')

hist(elsaw$psagf[elsaw$psagf>=0],
     main=labs[["psagf"]],xlab="Age")
cat('\n')

wtd.hist(elsaw$psagf[elsaw$psagf>=0 ],
         weight=elsaw$w10xwgt[elsaw$psagf>=0],
...
summary(elsaw$exlo80) 
cat('\n')

summary(elsaw$exlo80[elsaw$exlo80>0])  
cat('\n')

hist(elsaw$exlo80[elsaw$exlo80>=0 ],          
     main=(labs[["exlo80"]]),xlab="Age"
     )
cat('\n')

wtd.hist(elsaw$exlo80[elsaw$exlo80>=0 ],
         weight=elsaw$w10xwgt[elsaw$exlo80>=0],
        main=paste0(labs[["exlo80"]], " (weighted)"),xlab="Age")
cat('\n')
...
summary(elsaw$exlo90) 
cat('\n')

summary(elsaw$exlo90[elsaw$exlo90>0])  
cat('\n')

hist(elsaw$exlo90[elsaw$exlo90>=0 ],
         main=labs[["exlo90"]],xlab="Age")
cat('\n')

wtd.hist(elsaw$exlo90[elsaw$exlo90>=0 ],
         weight=elsaw$w10xwgt[elsaw$exlo90>=0],
         main=paste0(labs[["exlo90"]], " (weighted)"),xlab="Age")
cat('\n')
...
<!-- ###  5.1 Whether voted at  the 2019 GE by sex and age bands -->
...
<!-- print(kable( -->
<!-- ftable(pscedd.fr~VotGE19.fr+AgeCat2.fr+indsex.fr,data=elsaw) -->
<!-- #table(elsaw$pscedd.fr,elsaw$VotGE19.fr,elsaw$AgeCat2.fr,elsaw$indsex.fr) -->
<!-- ) -->
<!-- ) -->
...
This section presents the results of four series of stepwise logistic regression models of voter turnout at the 2019 general election:
  
1. Including turnout at previous GE, and socio-economic variables (+interaction of age and sex)
2. Including  SEV (+interaction of age and sex) only
3. Including  SEV (+interaction of age and sex)  as well as  independent variables: self-rated health, self-rated mental abilities, self-rated memory, financial situation, sense of loneliness, presence of an internet  connection, involvement in volunteering socio economic characteristiscs.
4. As 3. above,  alongside turnout at previous GE.
...
### 6.1 Models with socio-economic variables, including turnout at previous GE
...
#| output: asis
 ...
 elsaw$VotRec0.fr<-relevel(as.factor(elsaw$VotRec0.fr),ref=3)
...
regs1[["a11"]]<-glm(VotGE19.fr ~ VotRec0.fr,
                   weights=w10xwgt,
...
regs1[["a12"]]<-glm(VotGE19.fr ~ indsex.fr+VotRec0.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a13"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr+VotRec0.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 

regs1[["a13a"]]<-glm(VotGE19.fr ~ hhdtypb.fr +  AgeCat3.fr+indsex.fr+VotRec0.fr,
                    weights=w10xwgt,
                    elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a14"]]<-glm(VotGE19.fr ~ dimarr.fr+hhdtypb.fr +  AgeCat3.fr+indsex.fr+VotRec0.fr,
                   weights=w10xwgt, 
                   elsaw.r,family=binomial()) |>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a15"]]<-glm(VotGE19.fr ~ edqual.fr+ dimarr.fr+hhdtypb.fr +  AgeCat3.fr+indsex.fr+VotRec0.fr
                   ,  weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
tmp17<-glm(VotGE19.fr ~ wpdes.fr + 
             edqual.fr+ dimarr.fr+hhdtypb.fr +  AgeCat3.fr+indsex.fr+VotRec0.fr,
...
MS1<-tbl_merge(tbls=regs1,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("indsex.fr","AgeCat3.fr", "hhdtypb.fr", 
                                                 "dimarr.fr", "edqual.fr","wpdes.fr", "AgeCat3.fr * indsex.fr",
                                                  "VotRec0.fr","AIC", "logLik", "nobs" ))))
...
### 6.2 Models with SEV, without turnout at previous GE
...
tmp17<-glm(VotGE19.fr ~ 
             AgeCat3.fr*indsex.fr+hhdtypb.fr+dimarr.fr+
             edqual.fr+wpdes.fr,
           weights=w10xwgt,
           elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) 
...
MS2<-tbl_merge(tbls=regs2,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("indsex.fr","AgeCat3.fr", "hhdtypb.fr", 
                                                 "dimarr.fr", "edqual.fr","wpdes.fr", "AgeCat3.fr * indsex.fr","AIC", "logLik", "nobs" ))))
...
### 6.3 Models with SEV and independent variables, no turnout at previous GE
...
regs3[["a11"]]<-glm((VotGE19.fr) ~
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hehelf.fr,
                   weights=w10xwgt,elsaw.r,family=binomial()) |>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, 
                 digits=1,
                 include = hehelf.fr) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
regs3[["a14"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+ edqual.fr+wpdes.fr+
                     hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1,
...
tmp18<-glm(VotGE19.fr ~ 
             AgeCat3.fr*indsex.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr+pscede.fr+hobb.fr+erfvol.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1, 
                 include =c("AgeCat3.fr:indsex.fr", "hehelf.fr",
                            "cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","hobb.fr","erfvol.fr")) 
...
MS3<-tbl_merge(tbls=regs3,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("hehelf.fr",
                            "cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","hobb.fr","erfvol.fr","AgeCat3.fr * indsex.fr","AIC", "logLik", "nobs" ))))
...
### 6.4 Models with SEVs and IVs, and turnout at previous GE
...
regs4[["a11"]]<-glm((VotGE19.fr) ~
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hehelf.fr + VotRec0.fr,
                   weights=w10xwgt,elsaw.r,family=binomial()) |>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, 
                 digits=1,
                 include = c("hehelf.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs4[["a12"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr
                   +hehelf.fr+cfmap.fr+VotRec0.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, 
                 digits=1, 
                 include = c("hehelf.fr","cfmap.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs4[["a13"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hehelf.fr+cfmap.fr+cfmetm.fr+VotRec0.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, 
                 digits=1,                  
                 include = c("hehelf.fr","cfmap.fr","cfmetm.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 



regs4[["a14"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+ edqual.fr+wpdes.fr+
                     hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr+VotRec0.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1,
                 include = c("hehelf.fr","cfmap.fr","cfmetm.fr","iafcon.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs4[["a15"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr+pscede.fr+VotRec0.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, 
                 digits=1,
                 include = c("hehelf.fr","cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs4[["a16"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr+pscede.fr+hobb.fr+VotRec0.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1,
                 include = c("hehelf.fr","cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","hobb.fr","VotRec0.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs4[["a17"]]<-glm(VotGE19.fr ~ 
             indsex.fr+AgeCat3.fr+hhdtypb.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hehelf.fr +cfmap.fr+cfmetm.fr+iafcon.fr+pscede.fr+hobb.fr+erfvol.fr +VotRec0.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1, 
                 include =c("hehelf.fr","cfmap.fr","cfmetm.fr",
                            "iafcon.fr","pscede.fr","hobb.fr","erfvol.fr","VotRec0.fr")) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
tmp19<-glm(VotGE19.fr ~ 
  ...
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = T, 
                 digits=1, 
                 include =c("AgeCat3.fr:indsex.fr","hehelf.fr","cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","hobb.fr","erfvol.fr","VotRec0.fr")) 
...
MS4<-tbl_merge(tbls=regs4,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("hehelf.fr",
                            "cfmap.fr","cfmetm.fr","iafcon.fr","pscede.fr","hobb.fr","erfvol.fr","AgeCat3.fr * indsex.fr", "VotRec0.fr","AIC", "logLik", "nobs" ))))

MS4

```

#### Coefficients plot of Models 8 & 9 
