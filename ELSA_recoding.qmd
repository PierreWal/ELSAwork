...
subtitle: Exploratory analysis  - draft version 8
...
1. Added 4 series of models without past voting behaviour as covariate.
...
1. Add continuous IVs to the 4way tables in Section 5.1
...
exvars<-c("cfmap2","cfmap3", "cfmetm2", "cfmetm3", "dimarr", "edqual", "exrela", "hehelf2", "hehelf3", "hhdtypb","iafcon","sptraa", "tenureb","w10nssec5","wpdes") 
...
bivars19<-c("AgeCat2","AgeCat3", "cfmap2", "cfmap3", "cfmetm2", "cfmetm3","digran", "dimarr", "edqual", "erfvol", "erlvolba","erlvolpe","ethnic", "exrela", "headlba","headlbe","hehelf2","hehelf3", "hhdtypb","hobb","iafcon", "iafind","indsex", "pscedd", "pscede", "sclfcoa",  "scorgch","scorgpo","scorgrl","spcar",  "sptraa", "tenureb","VotGE01","VotGE05","VotGE15","VotGE17","VotRec2","VotRec0","VotRec01","w10nssec5","wpdes")     

      
reglab<-list(hobb.fr="Internet connection?",
                              ...
                              AgeGap.r="Perceived/actual age gap",
...
labs<-list(
AgeGap.r="Gap: actual and perceived age",
...
elsaw<-elsaw|>mutate(
  ...
  #   indager>=85  ~ "85+"
  # )),
  # AgeCat80.fr=as.factor(case_when(
  #   indager>=50 & indager<65 ~ "50-64",
...
elsaw.r<-elsaw |>filter(!is.na(indsex.fr) & !is.na(AgeCat3.fr) & !is.na(dimarr.fr) &
                          !is.na(edqual.fr)& !is.na(wpdes.fr) & !is.na(VotRec2.fr) & !is.na(w10xwgt) & !is.na(VotGE19.fr )) 

regs1<-list()

regs1[["a11"]]<-glm(VotGE19.fr ~ VotRec2.fr,
                   weights=w10xwgt,
...
regs1[["a12"]]<-glm(VotGE19.fr ~ indsex.fr+VotRec2.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a13"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr+VotRec2.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 

# regs1[["a13a"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr+VotRec2.fr,
#                     weights=w10xwgt,
#                     elsaw.r,family=binomial())|>
#   tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
#   add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a14"]]<-glm(VotGE19.fr ~ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr,
                   weights=w10xwgt, 
                   elsaw.r,family=binomial()) |>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs1[["a15"]]<-glm(VotGE19.fr ~ edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr
                   ,  weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
tmp16<-glm(VotGE19.fr ~ wpdes.fr + 
             edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr,
...
MS1<-tbl_merge(tbls=regs1,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("indsex.fr","AgeCat3.fr", 
                                                 "dimarr.fr", "edqual.fr","wpdes.fr",
                                                  "VotRec2.fr","AIC", "logLik", "nobs" ))))
...
elsaw.r<-elsaw |>filter(!is.na(tenureb.fr) & !is.na(indsex.fr) & !is.na(AgeCat3.fr) & !is.na(dimarr.fr) &
                          !is.na(edqual.fr)& !is.na(wpdes.fr) & !is.na(VotRec2.fr) & !is.na(w10xwgt) & !is.na(VotGE19.fr )) 

regs2<-list()

regs2[["a11"]]<-glm(VotGE19.fr ~ VotRec2.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs2[["a12"]]<-glm(VotGE19.fr ~ indsex.fr+VotRec2.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs2[["a13"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr+VotRec2.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 

# regs2[["a13a"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr+VotRec2.fr,
#                     weights=w10xwgt,
#                     elsaw.r,family=binomial())|>
#   tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
#   add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs2[["a14"]]<-glm(VotGE19.fr ~ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr,
                   weights=w10xwgt, 
                   elsaw.r,family=binomial()) |>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs2[["a15"]]<-glm(VotGE19.fr ~ edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr
                   ,  weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs2[["a16"]]<- tmp16 |>
 ...
 tmp17<-glm(VotGE19.fr ~ tenureb.fr+wpdes.fr +
              edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr+VotRec2.fr,
            weights=w10xwgt, elsaw.r,family=binomial())|>
   tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1)
 regs2[["a17"]]<- tmp17 |>
...
MS2<-tbl_merge(tbls=regs2,
    ...
    modify_table_body(~.x |>
                          dplyr::arrange(match(var_label,
                                               c("indsex.fr","AgeCat3.fr", 
                                                 "dimarr.fr", "edqual.fr","wpdes.fr","tenureb.fr",
                                                  "VotRec2.fr","AIC", "logLik", "nobs" ))))
...
elsaw.r<-elsaw |>filter(!is.na(indsex.fr) & !is.na(AgeCat3.fr)  & !is.na(dimarr.fr) &
                          !is.na(edqual.fr)& !is.na(wpdes.fr) & !is.na(hehelf3.fr) &
                          !is.na(cfmetm.fr)& !is.na(iafcon.fr)& !is.na(pscede.fr) &  !is.na(pscedd.fr) & !is.na(sclfcoa.fr) &
                          !is.na(hobb.fr) & !is.na(erlvolpe.fr) & !is.na(AgeGap.r) & !is.na(w10xwgt) & !is.na(VotGE19.fr ))  


regs3<-list()


regs3[["a11"]]<-glm((VotGE19.fr) ~VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr,
                   weights=w10xwgt,elsaw.r,family=binomial()) |>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = TRUE, 
                 digits=1,
                 include = c("hobb.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs3[["a12"]]<-glm(VotGE19.fr ~VotRec2.fr+ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                   hobb.fr+pscede.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:2,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = TRUE, 
                 digits=1, 
                 include = c("hobb.fr","pscede.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs3[["a13"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:3,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = TRUE, 
                 digits=1,                  
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 



regs3[["a14"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+ edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:4,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = T, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs3[["a15"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:5,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = TRUE, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs3[["a16"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr,
                     weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:6,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = T, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "VotRec2.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs3[["a17"]]<-glm(VotGE19.fr ~ VotRec2.fr+
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:7,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr", "VotRec2.fr")) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 

regs3[["a18"]]<-glm(VotGE19.fr ~ VotRec2.fr+
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr+erlvolpe.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 label = reglab[c(1:8,10)],
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr", "erlvolpe.fr", "VotRec2.fr")) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
tmp19<-glm(VotGE19.fr ~ VotRec2.fr+
             AgeCat3.fr+indsex.fr+dimarr.fr+edqual.fr+wpdes.fr+
                          hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr +erlvolpe.fr + AgeGap.r,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", 
                 label = reglab,
                 tidy_fun = broom.helpers::tidy_parameters,
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr",  "erlvolpe.fr",  "AgeGap.r", "VotRec2.fr")) 
...
MS3<-tbl_merge(tbls=regs3,
     ...
     modify_table_body(~.x |>
                      dplyr::arrange(match(var_label,
                                               c(as.character(reglab),"AIC", "logLik", "nobs" ))))
...
elsaw.r<-elsaw |>filter(!is.na(indsex.fr) & !is.na(tenureb.fr) & !is.na(AgeCat3.fr)  & !is.na(dimarr.fr) &
                          !is.na(edqual.fr)& !is.na(wpdes.fr) & !is.na(hehelf3.fr) &
                          !is.na(cfmetm.fr)& !is.na(iafcon.fr)& !is.na(pscede.fr) &  !is.na(pscedd.fr) & !is.na(sclfcoa.fr) &
                          !is.na(hobb.fr) & !is.na(erlvolpe.fr) & !is.na(AgeGap.r) & !is.na(w10xwgt) & !is.na(VotGE19.fr ))  
...
regs4[["a11"]]<-glm((VotGE19.fr) ~VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
                     hobb.fr,
                   weights=w10xwgt,elsaw.r,family=binomial()) |>  
  tbl_regression(ci_method="wald",
                                 label = reglab[c(1,10)],
...
regs4[["a12"]]<-glm(VotGE19.fr ~VotRec2.fr+ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
                   hobb.fr+pscede.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a13"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
                     hobb.fr+pscede.fr+hehelf3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a14"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+ edqual.fr+wpdes.fr+tenureb.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a15"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a16"]]<-glm(VotGE19.fr ~ VotRec2.fr+
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr,
                     weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a17"]]<-glm(VotGE19.fr ~ VotRec2.fr+
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
regs4[["a18"]]<-glm(VotGE19.fr ~ VotRec2.fr+
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr+erlvolpe.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
...
tmp19<-glm(VotGE19.fr ~ VotRec2.fr+
             AgeCat3.fr+indsex.fr+dimarr.fr+edqual.fr+wpdes.fr+tenureb.fr+
hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr +erlvolpe.fr + AgeGap.r,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald", 
                 label = reglab,
...
regs5[["a12"]]<-glm(VotGE19.fr ~ indsex.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs5[["a13"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
regs5[["a14"]]<-glm(VotGE19.fr ~ dimarr.fr +  AgeCat3.fr+indsex.fr,
                   weights=w10xwgt, 
                   elsaw.r,family=binomial()) |>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs5[["a15"]]<-glm(VotGE19.fr ~ edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr
                   ,  weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
tmp16<-glm(VotGE19.fr ~ wpdes.fr + 
             edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr,
           weights=w10xwgt, elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) 
...
regs6[["a12"]]<-glm(VotGE19.fr ~ indsex.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|> 
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs6[["a13"]]<-glm(VotGE19.fr ~ AgeCat3.fr+indsex.fr,
                   weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
regs6[["a14"]]<-glm(VotGE19.fr ~ dimarr.fr +  AgeCat3.fr+indsex.fr,
                   weights=w10xwgt, 
                   elsaw.r,family=binomial()) |>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs6[["a15"]]<-glm(VotGE19.fr ~ edqual.fr+ dimarr.fr +  AgeCat3.fr+indsex.fr
                   ,  weights=w10xwgt,
                   elsaw.r,family=binomial())|>
  tbl_regression(ci_method="wald", tidy_fun = broom.helpers::tidy_parameters,exponentiate = TRUE, digits=1) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
...
regs7[["a12"]]<-glm(VotGE19.fr ~ 
  ...
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = TRUE, 
                 digits=1, 
                 include = c("hobb.fr","pscede.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs7[["a13"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = TRUE, 
                 digits=1,                  
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 



regs7[["a14"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+ edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr,
                   weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = T, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs7[["a15"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr,
                   weights=w10xwgt, elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = TRUE, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs7[["a16"]]<-glm(VotGE19.fr ~ 
                     indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
                     hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr,
                     weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = T, 
                 digits=1,
                 include = c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr")) |>
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 


regs7[["a17"]]<-glm(VotGE19.fr ~ 
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr")) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 

regs7[["a18"]]<-glm(VotGE19.fr ~ 
             indsex.fr+AgeCat3.fr+dimarr.fr+edqual.fr+wpdes.fr+
             hobb.fr+pscede.fr+hehelf3.fr+iafcon.fr+cfmetm3.fr+pscedd.fr+sclfcoa.fr+erlvolpe.fr,
           weights=w10xwgt,elsaw.r,family=binomial())|>  
  tbl_regression(ci_method="wald",
                 ...
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr", "erlvolpe.fr")) |> 
  add_glance_table(include=c(AIC,logLik,nobs)) |> 
  add_significance_stars(hide_ci = F, hide_p = TRUE, hide_se = T) 
...
tmp19<-glm(VotGE19.fr ~ 
  ...
  tbl_regression(ci_method="wald", 
                 ...
                 exponentiate = T, 
                 digits=1, 
                 include =c("hobb.fr","pscede.fr", "hehelf3.fr","iafcon.fr", "cfmetm3.fr", "pscedd.fr", "sclfcoa.fr",  "erlvolpe.fr",  "AgeGap.r")) 
