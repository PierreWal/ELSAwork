# ELSA voting pattern study - version 7
...
### Data preparation - needs running only once 
# source("syntax/data_preparation.R")
...
for (v in bivars19[-c(1,2)]) {
  ...
  cat("#### ", labs[[fl]], " \n")
    # Use dplyr to group by AgeCat3.fr and indsex.fr
  elsaw |>
    filter(!is.na(elsaw$AgeCat3.fr) &  !is.na(elsaw[[fn]])) |>
    group_by(AgeCat3.fr, indsex.fr) |>
    group_split() |>
    lapply(function(sub_elsaw) {
      AgeCat3.fr_val <- as.character(unique(sub_elsaw$AgeCat3.fr))
      ...
      result <- matrix(paste0(tab, " (", sprintf("%.1f", row_perc), "%)"), 
      ...
      result_df <- rbind(result_df, c("Total", as.character(row_totals)))
      rownames(result_df)<-c(result_df$FactorLevel)
      result_df<-result_df[,-1]
      cat("\n **Age: **", AgeCat3.fr_val, ", **Sex: **", indsex.fr_val, " \n")
...
#### *Changes from version 6*
Changed  the order of the percentages from column to rows in Table 5.1 
